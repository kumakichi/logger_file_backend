defmodule LoggerFileBackend do
  ...
  defp level_cmp(level, min_level, match_level) do
    if match_level == true do
      Logger.compare_levels(level, min_level) == :eq
    else
  ...
  def handle_event(
        ...
        %{level: min_level, match_level: match_level, metadata_filter: metadata_filter} = state
      ) do
    if (is_nil(min_level) or level_cmp(level, min_level, match_level)) and
